...
feature_date <- function(df){
  
  df <- df %>% mutate(st_wday = wday(date, label = T),
                      st_year = year(date),
                      st_month = month(date))
  
  times <- as.POSIXlt(df$release, format = "%Y-%m-%dT%H:%M:%S", 
                      origin = "1970-01-01", tz = "GMT")
  
  df <- df %>% rename(st_hour = hour) %>% 
    mutate(release = date(times),
           year = year(release),
           month = month(release),
           wday = wday(release, label = T),
           hour = hour(times))
...
feature_increase <- function(df){
  
  get_increase <- function(df, name){
    
    df %>% filter(title == !!name) %>% 
      arrange(date) %>% 
      mutate(inc = target - lag(target))
  }
  
  A <- lapply(unique(df$title), get_increase, df = df)
  result <- do.call(rbind, A)
  return(result)
}

train <- feature_date(train)
test <- feature_date(test)
train <- feature_increase(train)
...
# So, I made these two features using "data scraped dates" and "podcast released date 

feature_episode <- function(df){
  
  foo <- df %>% 
    group_by(title) %>% 
    summarise(num_episode = n_distinct(release),
              flag_onair = ifelse((ymd(min(date)) - ymd(max(release))) < 0 & !num_episode == 1, 1, 0))
  
  df <- df %>% left_join(foo)
  
  get_flag_added <- function(df, name){
    
    df %>% filter(title == !!name) %>% 
      arrange(date) %>% 
      mutate(dif = (release - lag(release)) > 0,
             dif = ifelse(is.na(dif), FALSE, dif),
             flag_newepi_added = ifelse(flag_onair == 1 & dif, 1, 0)) %>% 
      select(-dif)
  }
  
  result <- lapply(unique(df$title), get_flag_added, df = df)
  result <- do.call(rbind, result)
  return(result)
}

train <- feature_episode(train)
test <- feature_episode(test)


# Duration feature
foo <- train %>% group_by(title) %>% summarise(duration = max(ymd(date)) - min(ymd(date)))
train <- train %>% left_join(foo %>% select(title, duration))
foo <- test %>% group_by(title) %>% summarise(duration = max(ymd(date)) - min(ymd(date)))
test <- test %>% left_join(foo %>% select(title, duration))
